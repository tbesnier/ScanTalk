...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(audio), torch.FloatTensor(vertices), torch.FloatTensor(template), torch.FloatTensor(np.array(mass)).float(), L.float(), torch.FloatTensor(np.array(evals)), torch.FloatTensor(np.array(evecs)), gradX.float(), gradY.float(), file_name
    ...
    for r, ds, fs in os.walk(audio_path):
        for f in tqdm(fs):
            if f.endswith("wav"):
                ...
                _, mass, L, evals, evecs, gradX, gradY = diffusion_net.geometry.compute_operators(torch.tensor(temp), faces=None, k_eig=128)
                data[key]["mass"] = mass
                data[key]["L"] = L
                data[key]["evals"] = evals
                data[key]["evecs"] = evecs
                data[key]["gradX"] = gradX
                data[key]["gradY"] = gradY
