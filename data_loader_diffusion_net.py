...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(audio), torch.FloatTensor(vertices), torch.FloatTensor(template), torch.FloatTensor(
            ...
            np.array(evecs)), gradX.float(), gradY.float(), file_name, faces.float()
...
def read_data(args):
    ...
    if args.dataset == "vocaset":
        ...
        reference = trimesh.load('/home/federico/Scrivania/ST/Data/Biwi/aligned_downsampled_templates/F1.obj',
                                 process=False)
        template_tri = reference.faces
