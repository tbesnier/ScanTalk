...
class DiffusionNetAutoencoder(nn.Module):
    def __init__(self, in_channels, out_channels, latent_channels, lstm_layers):
        ...
        self.mamba = Mamba(d_model=latent_channels, d_state=64, d_conv=16, expand=8)
        print("mamba parameters: ", count_parameters(self.mamba))
    ...
    def forward(self, audio, actor, vertices, mass, L, evals, evecs, gradX, gradY, faces, dataset, hks=None):
        ...
        for k in range(latent.shape[1]):        
            ...
            pred = pred_points + actor#[:, :, :3]
            pred_sequence = torch.vstack([pred_sequence, pred])
        return pred_sequence[1:, :, :]
    
    def forward_faster(self, audio, actor, vertices, mass, L, evals, evecs, gradX, gradY, faces, dataset, hks=None):
        hidden_states = self.audio_encoder(audio, dataset, frame_num=len(vertices)).last_hidden_state
        #pred_sequence = actor#[:, :, :3]
        audio_emb = self.audio_embedding(hidden_states)
        if hks is None:
        ...
        pred_points = self.decoder(combination, mass=mass, L=L, evals=evals, evecs=evecs, gradX=gradX, gradY=gradY, faces=faces)
        pred = pred_points + actor#[:, :, :3]
        #pred_sequence = torch.vstack([pred_sequence, pred])
        return pred#pred_sequence[1:, :, :]
