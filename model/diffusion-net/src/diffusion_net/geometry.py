...
def get_all_operators(verts_list, faces_list, k_eig, op_cache_dir=None, normals=None):
    ...
    for num, i in enumerate(inds):
        ...
        if normals is None:
            outputs = compute_operators(verts_list[i], faces_list, k_eig, op_cache_dir)
        else:
            outputs = compute_operators(verts_list[i], faces_list, k_eig, op_cache_dir, normals=normals[i])
        frames[i] = outputs[0]
        massvec[i] = outputs[1]
        L[i] = outputs[2]
        evals[i] = outputs[3]
