...
import torch
import igl
import model.njf.MeshProcessor as MeshProcessor
...
class SourceMesh:
    ...
    def __init__(self, source_v, source_f, use_wks=False, random_centering=False,
        ...
        self.source_v = source_v[:, :, :3]
        self.source_f = source_f
        #self.source_name = source_name
        #self.source_dir = source_dir_cache
    ...
    def __init_from_mesh_data(self):
        ...
        self.mesh_processor.prepare_differential_operators_for_use(torch.double)  # call 1
        self.source_vertices = torch.from_numpy(self.mesh_processor.get_vertices()).type(torch.double)
        #print(self.source_vertices.shape)
        ...
        self.centroids_and_normals = torch.from_numpy(
            centroid_points_and_normals).type(torch.double)
        ...
        #         #data *= scale
        #     data = data.unsqueeze(0).type(torch.double)
        #
