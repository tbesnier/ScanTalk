...
import sys
import lddmm_utils
from model.diffusion_net_encoder_lstm_njb_decoder import DiffusionNetAutoencoder
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                os.makedirs('/home/federico/Scrivania/ST/Data/saves/Meshes_DiffuserNet_Encoder_NJB/' + str(epoch), exist_ok=True)
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri)
                    mesh.export('/home/federico/Scrivania/ST/Data/saves/Meshes_DiffuserNet_Encoder_NJB/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
                ...
                os.makedirs('/home/federico/Scrivania/ST/Data/saves/Meshes_Training_DiffuserNet_Encoder_NJB/' + str(epoch), exist_ok=True)
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri)
                    mesh.export('/home/federico/Scrivania/ST/Data/saves/Meshes_Training_DiffuserNet_Encoder_NJB/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
        ...
        torch.save({'epoch': epoch,
            ...
            }, os.path.join(args.result_dir, 'd2d_ScanTalk_DiffuserNet_Encoder_njb.pth.tar'))
