...
def train(args):
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        for b, sample in pbar_talk:
            optim.zero_grad()
            audio, previous_audio = sample['audio'].to(device), sample['previous_audio'].to(device)
            frame, previous_frame = sample['frame'].to(device), sample['previous_frame'].to(device)
            next_frame = sample['next_frame'].to(device)
            actor = sample['actor'].to(device)
            next_frame_pred = d2d.forward(audio, frame, actor, previous_audio, previous_frame)
            loss = criterion(next_frame, next_frame_pred)  # + criterion(next_frame_pred - actor, next_frame - actor)
            loss.backward()
            optim.step()
            tloss += loss
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                for m in range(len(gen_seq)):
                    ...
                    mesh.export(
                        '../Data/VOCA/res/Results_Actor/Meshes/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
                ...
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri)
                    mesh.export(
                        '../Data/VOCA/res/Results_Actor/Meshes_Training/' + str(epoch) + '/frame_' + str(m).zfill(
                            3) + '.ply')
