...
import scipy
import sys
from model.diffusion_net_encoder_decoder_transformer import DiffusionNetAutoencoder
sys.path.append('/equilibrium/fnocentini/ScanTalk/model/diffusion-net_/src')
...
def train(args):
    ...
    temp_biwi = trimesh.load('/equilibrium/fnocentini/Datasets/Biwi_6/templates/M6.obj', process=False)
    template_vertices_biwi = torch.tensor(temp_biwi.vertices).to(args.device).float()
    template_tri_biwi = temp_biwi.faces
    
    temp_multiface = trimesh.load('/equilibrium/fnocentini/Datasets/Multiface/templates/20190828.ply', process=False)
    ...
    d2d = DiffusionNetAutoencoder(args.in_channels, args.in_channels, args.latent_channels, args.device).to(args.device)
    ...
    lip_mask_voca = scipy.io.loadmat('/equilibrium/fnocentini/Datasets/vocaset/FLAME_lips_idx.mat')
    ...
    lip_mask_biwi = np.load('/equilibrium/fnocentini/Datasets/Biwi_6/mouth_indices.npy')
    
    lip_mask_multiface = np.load('/equilibrium/fnocentini/Datasets/Multiface/mouth_indices.npy')
    ...
    criterion = nn.MSELoss()#Masked_Loss(lip_mask_voca, lip_mask_biwi, lip_mask_multiface)
    ...
    for epoch in range(starting_epoch, args.epochs):
        ...
        if epoch % 10 == 0:
            ...
            with torch.no_grad():
                ...
                frames, mass, L, evals, evecs, gradX, gradY = diffusion_net.geometry.compute_operators(torch.tensor(reference_voca.vertices).to('cpu'), faces=torch.tensor(template_tri_voca), k_eig=128)
                ...
                os.makedirs('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_VOCA_32/' + str(epoch), exist_ok=True)
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri_voca)
                    mesh.export('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_VOCA_32/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
                ...
                #Compute Operators for DiffuserNet
                frames, mass, L, evals, evecs, gradX, gradY = diffusion_net.geometry.compute_operators(torch.tensor(temp_biwi.vertices).to('cpu'), faces=torch.tensor(template_tri_biwi), k_eig=128)
                mass = torch.FloatTensor(np.array(mass)).float().to(device).unsqueeze(0)
                evals = torch.FloatTensor(np.array(evals)).to(device).unsqueeze(0)
                evecs = torch.FloatTensor(np.array(evecs)).to(device).unsqueeze(0)
                L = L.float().to(device).unsqueeze(0)
                ...
                os.makedirs('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_BIWI_32/' + str(epoch), exist_ok=True)
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri_biwi)
                    mesh.export('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_BIWI_32/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
                ...
                #Compute Operators for DiffuserNet
                frames, mass, L, evals, evecs, gradX, gradY = diffusion_net.geometry.compute_operators(torch.tensor(temp_multiface.vertices).to('cpu'), faces=torch.tensor(template_tri_multiface), k_eig=128)
                mass = torch.FloatTensor(np.array(mass)).float().to(device).unsqueeze(0)
                evals = torch.FloatTensor(np.array(evals)).to(device).unsqueeze(0)
                evecs = torch.FloatTensor(np.array(evecs)).to(device).unsqueeze(0)
                L = L.float().to(device).unsqueeze(0)
                ...
                os.makedirs('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_Multiface_32/' + str(epoch), exist_ok=True)
                for m in range(len(gen_seq)):
                    mesh = trimesh.Trimesh(gen_seq[m], template_tri_multiface)
                    mesh.export('/equilibrium/fnocentini/ScanTalk/saves/Meshes_DiffuserNet_Encoder_Decoder_Transformer_MSE_All_Multidataset_Multiface_32/' + str(epoch) + '/frame_' + str(m).zfill(3) + '.ply')
   
                
        torch.save({'epoch': epoch,
            'autoencoder_state_dict': d2d.state_dict(),
            'optimizer_state_dict': optim.state_dict(),
            }, os.path.join(args.result_dir, 'd2d_ScanTalk_DiffuserNet_Encoder_Decoder_Faces_MSE_Multidataset_VOCA_and_BIWI_and_Multiface_Transformer_32.pth.tar'))
...
def main():
    ...
    parser.add_argument("--reference_mesh_file_voca", type=str, default='/equilibrium/fnocentini/ScanTalk/template/flame_model/FLAME_sample.ply', help='path of the template')
    parser.add_argument("--epochs", type=int, default=200, help='number of epochs')
    parser.add_argument("--device", type=str, default="cuda:1")
    parser.add_argument("--result_dir", type=str, default='/equilibrium/fnocentini/ScanTalk/results')
    parser.add_argument("--sample_audio", type=str, default='/equilibrium/fnocentini/ScanTalk/photo.wav')
    parser.add_argument("--template_file_voca", type=str, default="/equilibrium/fnocentini/Datasets/vocaset/templates.pkl", help='faces to animate')
    parser.add_argument("--template_file_biwi", type=str, default="/equilibrium/fnocentini/Datasets/Biwi_6/templates", help='faces to animate')
    parser.add_argument("--template_file_multiface", type=str, default="/equilibrium/fnocentini/Datasets/Multiface/templates", help='faces to animate')
    ...
    parser.add_argument("--vertices_path_biwi", type=str, default="/equilibrium/fnocentini/Datasets/Biwi_6/vertices", help='path of the ground truth')
    parser.add_argument("--wav_path_multiface", type=str, default="/equilibrium/fnocentini/Datasets/Multiface/wav", help='path of the audio signals')
    parser.add_argument("--vertices_path_multiface", type=str, default="/equilibrium/fnocentini/Datasets/Multiface/vertices", help='path of the ground truth')
    parser.add_argument("--load_model", type=bool, default=False)
    parser.add_argument("--model_path", type=str, default='/equilibrium/fnocentini/ScanTalk/results/d2d_ScanTalk_DiffuserNet_Encoder_Decoder_Faces_MSE_Multidataset_VOCA_and_BIWI_and_Multiface_Transformer.pth.tar')
