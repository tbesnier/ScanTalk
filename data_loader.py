...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return torch.FloatTensor(audio), torch.FloatTensor(vertices), torch.FloatTensor(template), torch.FloatTensor(
            np.array(mass)).float(), L.float(), torch.FloatTensor(np.array(evals)), torch.FloatTensor(
            np.array(evecs)), gradX.float(), gradY.float(), file_name, faces.float(), dataset
...
def read_data(args):
    ...
    reference = trimesh.load('./data/vocaset/templates/FaceTalk_170725_00137_TA.ply',
                             process=False)
            ...
            if f.endswith("wav"):
                ...
                if subject_id not in subject_id_list:
                    ...
                    frame, mass, L, evals, evecs, gradX, gradY = diffusion_net.geometry.compute_operators(
                        torch.tensor(temp), faces=torch.tensor(template_tri), k_eig=128)
    ...
    # Read data from BIWI_6 dataset
    reference = trimesh.load('./data/BIWI_6/templates/F1.obj', process=False)
    template_tri = reference.faces
    ...
    for r, ds, fs in os.walk(audio_path_biwi):
        for f in tqdm(fs):
            if f.endswith("wav") and f[3] != 'e':
                ...
                temp = trimesh.load(os.path.join(args.template_file_biwi, subject_id + '.obj'), process=False).vertices
                data[key]["template"] = temp
    ...
    reference = trimesh.load('./data/multiface/templates/20180227.ply', process=False)
    template_tri = reference.faces
    ...
    for r, ds, fs in os.walk(audio_path_multiface):
        for f in tqdm(fs):
            if f.endswith("wav"):
                ...
                subject_id = key.split("_")[0]
                temp = trimesh.load(os.path.join(args.template_file_multiface, subject_id + '.ply'),
