...
for audio in os.listdir(audio_path):
    ...
    if os.path.exists(os.path.join(data_path, sentence)):
        ...
        for i in range(len(vertices)-1):
            print(j)       
            np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Frame/frame' + str(j).zfill(6) + '.npy', vertices[i])
            np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Next_Frame/frame' + str(j).zfill(6) + '.npy', vertices[i+1])
            np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Audio_Snippet_0/frame' + str(j).zfill(6) + '.npy', hidden_states[i+1])
            np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Audio_Snippet_1/frame' + str(j).zfill(6) + '.npy', hidden_states[i])
            if i != 0:
                np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Audio_Snippet_2/frame' + str(j).zfill(6) + '.npy', hidden_states[i-1])
            else:
                np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Audio_Snippet_2/frame' + str(j).zfill(6) + '.npy', hidden_states[i])
                
            np.save('/home/federico/Scrivania/ScanTalk/Consecutive_Dataset_WavLM/Actor/frame' + str(j).zfill(6) + '.npy', template)
            j+=1
